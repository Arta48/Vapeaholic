...
public class RegenerationParticle extends SimpleAnimatedParticle {
    private RegenerationParticle(final ClientLevel level, final double x, final double y, final double z, final double xa, final double ya, final double za, final SpriteSet sprites) {
    ...
    public static RegenerationProvider Provider(FabricSpriteSet fabricSpriteSet) {
        return new RegenerationProvider(fabricSpriteSet);
    }

    @Environment(EnvType.CLIENT)
    public static class RegenerationProvider implements ParticleProvider<SimpleParticleType> {
        private final SpriteSet sprites;

        public RegenerationProvider(final SpriteSet sprites) {
            this.sprites = sprites;
        }

        public Particle createParticle(final SimpleParticleType options, final ClientLevel level, final double x, final double y, final double z, final double xAux, final double yAux, final double zAux, final RandomSource random) {
            return new RegenerationParticle(level, x, y, z, xAux, yAux, zAux, this.sprites);
        }
    }
