...
public class SlownessParticle extends SimpleAnimatedParticle {
    private SlownessParticle(final ClientLevel level, final double x, final double y, final double z, final double xa, final double ya, final double za, final SpriteSet sprites) {
    ...
    public static SlownessProvider Provider(FabricSpriteSet fabricSpriteSet) {
        return new SlownessProvider(fabricSpriteSet);
    }

    @Environment(EnvType.CLIENT)
    public static class SlownessProvider implements ParticleProvider<SimpleParticleType> {
        private final SpriteSet sprites;

        public SlownessProvider(final SpriteSet sprites) {
            this.sprites = sprites;
        }

        public Particle createParticle(final SimpleParticleType options, final ClientLevel level, final double x, final double y, final double z, final double xAux, final double yAux, final double zAux, final RandomSource random) {
            return new SlownessParticle(level, x, y, z, xAux, yAux, zAux, this.sprites);
        }
    }
