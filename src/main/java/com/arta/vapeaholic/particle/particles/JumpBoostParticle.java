...
public class JumpBoostParticle extends SimpleAnimatedParticle {
    private JumpBoostParticle(final ClientLevel level, final double x, final double y, final double z, final double xa, final double ya, final double za, final SpriteSet sprites) {
    ...
    public static JumpBoostProvider Provider(FabricSpriteSet fabricSpriteSet) {
        return new JumpBoostProvider(fabricSpriteSet);
    }

    @Environment(EnvType.CLIENT)
    public static class JumpBoostProvider implements ParticleProvider<SimpleParticleType> {
        private final SpriteSet sprites;

        public JumpBoostProvider(final SpriteSet sprites) {
            this.sprites = sprites;
        }

        public Particle createParticle(final SimpleParticleType options, final ClientLevel level, final double x, final double y, final double z, final double xAux, final double yAux, final double zAux, final RandomSource random) {
            return new JumpBoostParticle(level, x, y, z, xAux, yAux, zAux, this.sprites);
        }
    }
