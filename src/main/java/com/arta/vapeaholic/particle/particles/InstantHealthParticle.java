...
public class InstantHealthParticle extends SimpleAnimatedParticle {
    private InstantHealthParticle(final ClientLevel level, final double x, final double y, final double z, final double xa, final double ya, final double za, final SpriteSet sprites) {
    ...
    public static InstantHealthProvider Provider(FabricSpriteSet fabricSpriteSet) {
        return new InstantHealthProvider(fabricSpriteSet);
    }

    @Environment(EnvType.CLIENT)
    public static class InstantHealthProvider implements ParticleProvider<SimpleParticleType> {
        private final SpriteSet sprites;

        public InstantHealthProvider(final SpriteSet sprites) {
            this.sprites = sprites;
        }

        public Particle createParticle(final SimpleParticleType options, final ClientLevel level, final double x, final double y, final double z, final double xAux, final double yAux, final double zAux, final RandomSource random) {
            return new InstantHealthParticle(level, x, y, z, xAux, yAux, zAux, this.sprites);
        }
    }
