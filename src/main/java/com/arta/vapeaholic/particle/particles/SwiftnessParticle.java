...
public class SwiftnessParticle extends SimpleAnimatedParticle {
    private SwiftnessParticle(final ClientLevel level, final double x, final double y, final double z, final double xa, final double ya, final double za, final SpriteSet sprites) {
    ...
    public static SwiftnessProvider Provider(FabricSpriteSet fabricSpriteSet) {
        return new SwiftnessProvider(fabricSpriteSet);
    }

    @Environment(EnvType.CLIENT)
    public static class SwiftnessProvider implements ParticleProvider<SimpleParticleType> {
        private final SpriteSet sprites;

        public SwiftnessProvider(final SpriteSet sprites) {
            this.sprites = sprites;
        }

        public Particle createParticle(final SimpleParticleType options, final ClientLevel level, final double x, final double y, final double z, final double xAux, final double yAux, final double zAux, final RandomSource random) {
            return new SwiftnessParticle(level, x, y, z, xAux, yAux, zAux, this.sprites);
        }
    }
