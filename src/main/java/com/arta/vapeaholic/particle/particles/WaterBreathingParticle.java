...
public class WaterBreathingParticle extends SimpleAnimatedParticle {
    private WaterBreathingParticle(final ClientLevel level, final double x, final double y, final double z, final double xa, final double ya, final double za, final SpriteSet sprites) {
    ...
    public static WaterBreathingProvider Provider(FabricSpriteSet fabricSpriteSet) {
        return new WaterBreathingProvider(fabricSpriteSet);
    }

    @Environment(EnvType.CLIENT)
    public static class WaterBreathingProvider implements ParticleProvider<SimpleParticleType> {
        private final SpriteSet sprites;

        public WaterBreathingProvider(final SpriteSet sprites) {
            this.sprites = sprites;
        }

        public Particle createParticle(final SimpleParticleType options, final ClientLevel level, final double x, final double y, final double z, final double xAux, final double yAux, final double zAux, final RandomSource random) {
            return new WaterBreathingParticle(level, x, y, z, xAux, yAux, zAux, this.sprites);
        }
    }
