...
public class FireResistanceParticle extends SimpleAnimatedParticle {
    private FireResistanceParticle(final ClientLevel level, final double x, final double y, final double z, final double xa, final double ya, final double za, final SpriteSet sprites) {
    ...
    public static FireResistanceProvider Provider(FabricSpriteSet fabricSpriteSet) {
        return new FireResistanceProvider(fabricSpriteSet);
    }

    @Environment(EnvType.CLIENT)
    public static class FireResistanceProvider implements ParticleProvider<SimpleParticleType> {
        private final SpriteSet sprites;

        public FireResistanceProvider(final SpriteSet sprites) {
            this.sprites = sprites;
        }

        public Particle createParticle(final SimpleParticleType options, final ClientLevel level, final double x, final double y, final double z, final double xAux, final double yAux, final double zAux, final RandomSource random) {
            return new FireResistanceParticle(level, x, y, z, xAux, yAux, zAux, this.sprites);
        }
    }
