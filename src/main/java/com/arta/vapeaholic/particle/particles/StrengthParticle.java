...
public class StrengthParticle extends SimpleAnimatedParticle {
    private StrengthParticle(final ClientLevel level, final double x, final double y, final double z, final double xa, final double ya, final double za, final SpriteSet sprites) {
    ...
    public static StrengthProvider Provider(FabricSpriteSet fabricSpriteSet) {
        return new StrengthProvider(fabricSpriteSet);
    }

    @Environment(EnvType.CLIENT)
    public static class StrengthProvider implements ParticleProvider<SimpleParticleType> {
        private final SpriteSet sprites;

        public StrengthProvider(final SpriteSet sprites) {
            this.sprites = sprites;
        }

        public Particle createParticle(final SimpleParticleType options, final ClientLevel level, final double x, final double y, final double z, final double xAux, final double yAux, final double zAux, final RandomSource random) {
            return new StrengthParticle(level, x, y, z, xAux, yAux, zAux, this.sprites);
        }
    }
