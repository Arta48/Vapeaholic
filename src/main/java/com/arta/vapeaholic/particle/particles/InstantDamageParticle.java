...
public class InstantDamageParticle extends SimpleAnimatedParticle {
    private InstantDamageParticle(final ClientLevel level, final double x, final double y, final double z, final double xa, final double ya, final double za, final SpriteSet sprites) {
    ...
    public static InstantDamageProvider Provider(FabricSpriteSet fabricSpriteSet) {
        return new InstantDamageProvider(fabricSpriteSet);
    }

    @Environment(EnvType.CLIENT)
    public static class InstantDamageProvider implements ParticleProvider<SimpleParticleType> {
        private final SpriteSet sprites;

        public InstantDamageProvider(final SpriteSet sprites) {
            this.sprites = sprites;
        }

        public Particle createParticle(final SimpleParticleType options, final ClientLevel level, final double x, final double y, final double z, final double xAux, final double yAux, final double zAux, final RandomSource random) {
            return new InstantDamageParticle(level, x, y, z, xAux, yAux, zAux, this.sprites);
        }
    }
