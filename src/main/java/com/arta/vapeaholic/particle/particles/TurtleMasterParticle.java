...
public class TurtleMasterParticle extends SimpleAnimatedParticle {
    private TurtleMasterParticle(final ClientLevel level, final double x, final double y, final double z, final double xa, final double ya, final double za, final SpriteSet sprites) {
    ...
    public static TurtleMasterProvider Provider(FabricSpriteSet fabricSpriteSet) {
        return new TurtleMasterProvider(fabricSpriteSet);
    }

    @Environment(EnvType.CLIENT)
    public static class TurtleMasterProvider implements ParticleProvider<SimpleParticleType> {
        private final SpriteSet sprites;

        public TurtleMasterProvider(final SpriteSet sprites) {
            this.sprites = sprites;
        }

        public Particle createParticle(final SimpleParticleType options, final ClientLevel level, final double x, final double y, final double z, final double xAux, final double yAux, final double zAux, final RandomSource random) {
            return new TurtleMasterParticle(level, x, y, z, xAux, yAux, zAux, this.sprites);
        }
    }
