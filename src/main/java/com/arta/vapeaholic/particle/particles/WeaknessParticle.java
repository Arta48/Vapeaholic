...
public class WeaknessParticle extends SimpleAnimatedParticle {
    private WeaknessParticle(final ClientLevel level, final double x, final double y, final double z, final double xa, final double ya, final double za, final SpriteSet sprites) {
    ...
    public static WeaknessProvider Provider(FabricSpriteSet fabricSpriteSet) {
        return new WeaknessProvider(fabricSpriteSet);
    }

    @Environment(EnvType.CLIENT)
    public static class WeaknessProvider implements ParticleProvider<SimpleParticleType> {
        private final SpriteSet sprites;

        public WeaknessProvider(final SpriteSet sprites) {
            this.sprites = sprites;
        }

        public Particle createParticle(final SimpleParticleType options, final ClientLevel level, final double x, final double y, final double z, final double xAux, final double yAux, final double zAux, final RandomSource random) {
            return new WeaknessParticle(level, x, y, z, xAux, yAux, zAux, this.sprites);
        }
    }
