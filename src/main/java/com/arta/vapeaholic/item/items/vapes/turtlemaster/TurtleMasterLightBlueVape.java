package com.arta.vapeaholic.item.items.vapes.turtlemaster;
...
public class TurtleMasterLightBlueVape extends Item {

    public TurtleMasterLightBlueVape                                                                                                                          () {
        super(ModVariables.VapeSettings
            .setId(ResourceKey.create(Registries.ITEM, Identifier.fromNamespaceAndPath(Vapeaholic.MOD_ID,"turtle_master_light_blue_vape")))
            .modelId(Identifier.fromNamespaceAndPath(Vapeaholic.MOD_ID,"light_blue_vape"))
            .overrideDescription("item.vapeaholic.light_blue_vape")
        );
    }
    ...
    public void appendHoverText(final @NonNull ItemStack itemStack, final @NonNull TooltipContext context, final @NonNull TooltipDisplay display, final Consumer<Component> builder, final @NonNull TooltipFlag tooltipFlag) {
        builder.accept(Component.translatable("item.vapeaholic.turtle_master_vape.tooltip1"));
        builder.accept(Component.translatable("item.vapeaholic.turtle_master_vape.tooltip2"));
    }
    ...
    public @NonNull InteractionResult use(final @NonNull Level level, final Player player, final @NonNull InteractionHand hand) {
        ...
        effectList.add(MobEffects.SLOWNESS);
        effectList.add(MobEffects.RESISTANCE);
        RightClickProcedure.execute(level, player.getX(), player.getY(), player.getZ(), player, itemStack, ModParticleTypes.TURTLE_MASTER_PARTICLE, effectList);
