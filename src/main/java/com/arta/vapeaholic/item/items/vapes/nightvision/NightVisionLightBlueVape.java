package com.arta.vapeaholic.item.items.vapes.nightvision;
...
public class NightVisionLightBlueVape extends Item {

    public NightVisionLightBlueVape() {
        super(ModVariables.VapeSettings
            .setId(ResourceKey.create(Registries.ITEM, Identifier.fromNamespaceAndPath(Vapeaholic.MOD_ID,"night_vision_light_blue_vape")))
    ...
    public void appendHoverText(final @NonNull ItemStack itemStack, final @NonNull TooltipContext context, final @NonNull TooltipDisplay display, final Consumer<Component> builder, final @NonNull TooltipFlag tooltipFlag) {
        builder.accept(Component.translatable("item.vapeaholic.night_vision_vape.tooltip"));
    }
    ...
    public @NonNull InteractionResult use(final @NonNull Level level, final Player player, final @NonNull InteractionHand hand) {
        ...
        effectList.add(MobEffects.NIGHT_VISION);
        RightClickProcedure.execute(level, player.getX(), player.getY(), player.getZ(), player, itemStack, ModParticleTypes.NIGHT_VISION_PARTICLE, effectList);
