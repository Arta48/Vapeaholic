package com.arta.vapeaholic.item.items.vapes.jumpboost;
...
public class JumpBoostLightBlueVape extends Item {

    public JumpBoostLightBlueVape() {
        super(ModVariables.VapeSettings
            .setId(ResourceKey.create(Registries.ITEM, Identifier.fromNamespaceAndPath(Vapeaholic.MOD_ID,"jump_boost_light_blue_vape")))
    ...
    public void appendHoverText(final @NonNull ItemStack itemStack, final @NonNull TooltipContext context, final @NonNull TooltipDisplay display, final Consumer<Component> builder, final @NonNull TooltipFlag tooltipFlag) {
        builder.accept(Component.translatable("item.vapeaholic.jump_boost_vape.tooltip"));
    }
    ...
    public @NonNull InteractionResult use(final @NonNull Level level, final Player player, final @NonNull InteractionHand hand) {
        ...
        effectList.add(MobEffects.JUMP_BOOST);
        RightClickProcedure.execute(level, player.getX(), player.getY(), player.getZ(), player, itemStack, ModParticleTypes.JUMP_BOOST_PARTICLE, effectList);
